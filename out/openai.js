...
exports.makeOpenAIClient = makeOpenAIClient;
exports.generateCommitMessage = generateCommitMessage;
const openai_1 = require("openai");
function makeOpenAIClient(apiKey) {
    const key = apiKey || process.env.OPENAI_API_KEY;
    if (!key) {
        throw new Error("Ingen OpenAI-nyckel hittades. Sätt `gitCommitGPT.openaiApiKey` i inställningar eller exportera miljövariabeln OPENAI_API_KEY.");
    }
...
async function generateCommitMessage(opts) {
    ...
    const styleHint = style === "conventional"
        ? (language === "sv"
...
`.trim();
    ...
    if (!text)
        throw new Error("Kunde inte generera commit-meddelande.");
    return text;
}
